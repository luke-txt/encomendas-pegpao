...
const SITE_URL   = 'https://encomendas-pegpao.vercel.app';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const buscaRes  = await fetch(`${ASAAS_BASE}/customers?email=${encodeURIComponent(clienteEmail)}&limit=1`, { headers });
    ...
    // ── 2. Data de vencimento (hoje + 1 dia) ─────────────────────────────
    const venc = new Date();
    venc.setDate(venc.getDate() + 1);
    const dueDate = venc.toISOString().split('T')[0];

    // ── 3. Cria a cobrança com URL de callback ────────────────────────────
